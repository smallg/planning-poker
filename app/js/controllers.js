...
function LobbyCtrl($scope, $location, $cookieStore, socket) {
    ...
    $scope.enterRoom = function (room, username) {
        ...
        $scope.disableButtons = true;
        socket.emit('room info', {roomUrl: room, username: username}, function (response) {
            if (response.error) {
                $scope.disableButtons = false;
...
LobbyCtrl.$inject = ['$scope', '$location', '$cookieStore', 'socket'];

function RoomCtrl($scope, $routeParams, $timeout, $cookieStore, socket) {
    ...
    var myConnectionHash = function () {
        return _.find($scope.connections, function (c) {
            return c.sessionId === $scope.sessionId;
        });
    };

    var myVoteHash = function () {
        return _.find($scope.votes, function (c) {
            return c.sessionId === $scope.sessionId;
        });
    };
    ...
    var setLocalVote = function (vote) {
        ...
        if (!voteHash) {
            ...
            $scope.votes.push({sessionId: $scope.sessionId, vote: vote, username: $scope.username});
        } else {
            if (vote) {
                voteHash.vote = vote;
            } else {
    ...
    $scope.configureRoom = function () {
        ...
        socket.on('room joined', function () {
            ...
            this.emit('room info', {roomUrl: $scope.roomId, username: $scope.username}, function (response) {
    ...
    $scope.setCardPack = function (cardPack) {
        ...
        socket.emit('set card pack', {roomUrl: $scope.roomId, cardPack: cardPack, username: $scope.username});
    };
    ...
    $scope.vote = function (vote) {
        if ($scope.myVote !== vote) {
            if (!votingFinished() && $scope.voter) {
                ...
                socket.emit('vote', {roomUrl: $scope.roomId, vote: vote, sessionId: $scope.sessionId, username: $scope.username}, function (response) {
    ...
    $scope.unvote = function (sessionId) {
        if (sessionId === $scope.sessionId) {
            if (!votingFinished()) {
                ...
                socket.emit('unvote', {roomUrl: $scope.roomId, sessionId: $scope.sessionId, username: $scope.username}, function (response) {
                    processMessage(response);
                });
            }
        }
    ...
    $scope.resetVote = function () {
        ...
        socket.emit('reset vote', {roomUrl: $scope.roomId, username: $scope.username}, function (response) {
            processMessage(response);
        });
    };
    ...
    $scope.forceReveal = function () {
        ...
        socket.emit('force reveal', {roomUrl: $scope.roomId, username: $scope.username}, function (response) {
            processMessage(response);
        });
    };
    ...
    $scope.toggleVoter = function () {
        ...
        socket.emit('toggle voter', {roomUrl: $scope.roomId, voter: $scope.voter, sessionId: $scope.sessionId, username: $scope.username}, function (response) {
            processMessage(response);
        });
    };
...
RoomCtrl.$inject = ['$scope', '$routeParams', '$timeout', '$cookieStore', 'socket'];
