...
Room.prototype.enter = function (socket, data) {
    ...
    if (this.connections[data.sessionId]) {
        ...
        this.connections[data.sessionId] = {sessionId: data.sessionId, socketId: socket.id, vote: null, voter: true, username: data.username};
    }
};
...
Room.prototype.json = function () {
    return {
        ...
        connections: _.filter(this.connections, function (c) {
            return c.socketId
        })
    };
};
